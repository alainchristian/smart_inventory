...
class RwandaShoeBusinessSeeder extends Seeder
{
    ...
    private function seedTransporters(): mixed
    {
    ...
    // ═══════════════════════════════════════════════════════
    //  PRODUCTS
    //  All prices stored as direct RWF (whole numbers)
    ...
    private function seedProducts(array $cats): mixed
    {
        $products = collect();
        ...
        foreach ($afSizes as $sz) {
            $products->push(Product::create([
                ...
                'items_per_box'       => 6,
                'purchase_price'      => 35000,   // RWF 35,000
                'selling_price'       => 55000,   // RWF 55,000
                'box_selling_price'   => 315000,  // RWF 315,000
    ...
    private function seedTransfers(array $locations, array $users, mixed $transporters, mixed $products): void
    {
        ...
        foreach ($scenarios as $sc) {
            ...
            foreach ($selectedProducts as $product) {
                ...
                $receivedQty  = $status === 'received' ? $requestedQty : 0;

                TransferItem::create([
                    'transfer_id'        => $transfer->id,
                    'product_id'         => $product->id,
                    'quantity_requested' => $requestedQty,
                    'quantity_shipped'   => in_array($status, ['in_transit','delivered','received']) ? $requestedQty : 0,
    ...
    private function printSummary(array $users, array $locations, mixed $products): void
    {
        ...
        // Price sample
        $this->command->info('  💰 PRICE SAMPLES (stored as direct RWF):');
        $this->command->info('      Nike AF1 Size 42 → sell: RWF 55,000 → DB: 55000');
        $this->command->info('      School Shoe 38   → sell: RWF 20,000 → DB: 20000');
        $this->command->info('      Socks 3-Pack     → sell: RWF  2,500 → DB:  2500');
        $this->command->info('');
        $this->command->info(str_repeat('═', 56));
        $this->command->info('');
    }

<x-app-layout>
    <div class="mb-4 sm:mb-6 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold" style="color:var(--text);">Add Expense</h1>
            <p class="mt-0.5 text-xs" style="color:var(--text-dim);">Record an operational expense against today's session.</p>
        </div>
    </div>
    ...
    @if ($activeSession)
        <div style="background:var(--surface);border-radius:var(--r);box-shadow:var(--shadow-card);padding:20px;max-width:560px;">
            <livewire:shop.day-close.add-expense :dailySessionId="$activeSession->id" />
        </div>

        <div style="background:var(--surface);border-radius:var(--r);box-shadow:var(--shadow-card);margin-top:20px;">
            <div style="padding:14px 20px;border-bottom:1px solid var(--border);font-size:13px;font-weight:700;color:var(--text);">Today's Expenses</div>
            <livewire:shop.day-close.expense-list :dailySessionId="$activeSession->id" />
        </div>
    @else
        <div class="rounded-xl p-8 text-center" style="background:var(--surface2);border:1px solid var(--border);">
            ...
            <div class="text-xs mb-4" style="color:var(--text-dim);">Open today's session before recording expenses.</div>
            <a href="{{ route('shop.session.open') }}"

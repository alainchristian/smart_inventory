...
$svg = $paths[$name] ?? $paths['x'];
@endphp
<svg {{ $attributes->merge(['width' => $size, 'height' => $size]) }} fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" style="flex-shrink:0;display:inline-block;vertical-align:middle">{!! $svg !!}</svg>

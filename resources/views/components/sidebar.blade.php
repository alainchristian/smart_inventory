                ...
                <div class="space-y-1">
                    @if(auth()->user()->isOwner())
                        ...
                        <a href="{{ route('shop.pos') }}"
                           class="flex items-center space-x-2.5 px-3 py-2 rounded-lg transition-all {{ request()->routeIs('shop.pos') ? 'border' : '' }}"
                           style="{{ request()->routeIs('shop.pos') ? 'background: var(--accent-glow); color: var(--accent); border-color: var(--accent);' : 'color: var(--text-sub);' }}"
                           onmouseover="if (!this.classList.contains('border')) { this.style.background='var(--surface2)'; this.style.color='var(--text)'; }"
                           onmouseout="if (!this.classList.contains('border')) { this.style.background='transparent'; this.style.color='var(--text-sub)'; }">
                            <svg class="w-[18px] h-[18px] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                            </svg>
                            <span class="text-[13px] font-medium">Point of Sale</span>
                        </a>

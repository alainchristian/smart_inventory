...
<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div>
                ...
                <p class="text-gray-500 text-2xl mt-0.5">Transfer efficiency and logistics analytics</p>
            </div>

            <div class="flex flex-col sm:flex-row gap-2">
                <!-- Status Filter -->
                <select wire:model.live="statusFilter"
                        class="px-3 py-2 border border-gray-300 rounded-lg text-2xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                ...
                <div class="flex gap-2">
                    <button wire:click="setDateRange('today')"
                            class="px-3 py-2 text-2xl {{ $dateFrom === now()->format('Y-m-d') && $dateTo === now()->format('Y-m-d') ? 'bg-purple-600 text-white' : 'bg-white text-gray-700 border border-gray-300' }} rounded-lg hover:bg-purple-700 hover:text-white transition-colors">
                        Today
                    </button>
                    <button wire:click="setDateRange('week')"
                            class="px-3 py-2 text-2xl {{ $dateFrom === now()->subDays(7)->format('Y-m-d') ? 'bg-purple-600 text-white' : 'bg-white text-gray-700 border border-gray-300' }} rounded-lg hover:bg-purple-700 hover:text-white transition-colors">
                        Week
                    </button>
                    <button wire:click="setDateRange('month')"
                            class="px-3 py-2 text-2xl {{ $dateFrom === now()->subDays(30)->format('Y-m-d') ? 'bg-purple-600 text-white' : 'bg-white text-gray-700 border border-gray-300' }} rounded-lg hover:bg-purple-700 hover:text-white transition-colors">
                        Month
                    </button>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow text-white">
            <div class="flex items-center justify-between mb-2">
                <span class="text-2xl font-bold uppercase opacity-90">Total Transfers</span>
            ...
            <p class="text-2xl opacity-75 mt-2">Total transfers in period</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow text-white">
            <div class="flex items-center justify-between mb-2">
                <span class="text-2xl font-bold uppercase opacity-90">Avg Completion</span>
                <svg class="w-6 h-6 opacity-75" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
            </div>
            <p class="text-2xl font-bold">{{ number_format($this->transferKpis['avg_completion_hours'], 1) }}h</p>
            <p class="text-2xl opacity-75 mt-2">Average hours to complete</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow text-white">
            <div class="flex items-center justify-between mb-2">
                <span class="text-2xl font-bold uppercase opacity-90">Discrepancy Rate</span>
                <svg class="w-6 h-6 opacity-75" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                </svg>
            </div>
            <p class="text-2xl font-bold">{{ number_format($this->transferKpis['discrepancy_rate'], 2) }}%</p>
            <p class="text-2xl opacity-75 mt-2">Transfers with discrepancies</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow text-white">
            <div class="flex items-center justify-between mb-2">
                <span class="text-2xl font-bold uppercase opacity-90">In Transit</span>
                <svg class="w-6 h-6 opacity-75" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                </svg>
            </div>
            <p class="text-2xl font-bold">{{ number_format($this->transferKpis['in_transit_count']) }}</p>
            <p class="text-2xl opacity-75 mt-2">Currently in transit</p>
        </div>
    </div>

    <!-- Transfer Volume Trend -->
    <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200 mb-6">
        <h2 class="text-2xl font-bold text-gray-900 mb-4">Transfer Volume Trend</h2>
        <div id="transferVolumeTrendChart" style="height: 300px;"></div>
    </div>

    <!-- Transfer Routes Matrix -->
    <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200 mb-6">
        <h2 class="text-2xl font-bold text-gray-900 mb-4">Transfer Routes (Warehouse → Shop)</h2>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-right">
                                @if($route['discrepancy_count'] > 0)
                                    <span class="px-2 py-1 text-2xl rounded-full bg-amber-100 text-amber-800">
                            ...
                            <td class="px-3 py-2 text-right">
                                <span class="px-2 py-1 text-2xl rounded-full
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-4">Status Distribution</h2>
            <div id="statusDistributionChart" style="height: 300px;"></div>
        </div>

        <!-- Completion Time Distribution -->
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-4">Completion Time Distribution</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-4">Most Transferred Products</h2>
        ...
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-4">Recent Discrepancies</h2>
            <div class="overflow-y-auto" style="max-height: 350px;">
                <div class="space-y-2">
                    @forelse($this->recentDiscrepancies as $transfer)
                        <div class="p-3 bg-amber-50 border-l-4 border-amber-500 rounded-lg">
                            <div class="flex items-start justify-between mb-1">
                                <span class="text-2xl font-bold text-gray-900">{{ $transfer['transfer_number'] }}</span>
                                <span class="text-2xl text-gray-500">{{ \Carbon\Carbon::parse($transfer['created_at'])->format('M d, Y') }}</span>
                            </div>
                            <div class="text-2xl text-gray-700 mb-1">
                                <strong>From:</strong> {{ $transfer['from_warehouse'] }} →
                                <strong>To:</strong> {{ $transfer['to_shop'] }}
                            </div>
                            @if($transfer['discrepancy_notes'])
                                <p class="text-2xl text-gray-600 mt-1">{{ Str::limit($transfer['discrepancy_notes'], 80) }}</p>
                            @endif
                        </div>
                    @empty
                        <p class="text-2xl text-center text-gray-500 py-4">No recent discrepancies</p>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
        <h2 class="text-2xl font-bold text-gray-900 mb-4">Warehouse Efficiency</h2>
        <div class="overflow-x-auto">
            <table class="min-w-full text-2xl">
                <thead class="bg-gray-50">
                    <tr>

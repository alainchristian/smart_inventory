...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="bg-gradient-to-r from-indigo-600 to-blue-600 rounded-lg shadow-lg p-4 md:p-6 text-white">
        <div class="flex items-center gap-3 md:gap-4 mb-3 md:mb-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-indigo-100 text-sm md:text-base">Scan in any order - mix products freely</p>
            </div>
        </div>
            ...
            <div class="bg-white text-gray-900 rounded-lg p-3 md:p-4">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 md:gap-3">
                    ...
                    <button type="button"
                            ...
                            class="px-4 md:px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium transition-colors text-base md:text-lg">
                        Confirm
                    </button>
                    <button type="button"
    ...
    <!-- Packing Summary -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h3 class="text-lg md:text-xl font-semibold text-gray-900 mb-3 md:mb-4">Packing Summary</h3>
            ...
            <div class="inline-block min-w-full align-middle">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-3 md:px-6 py-2 md:py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">Product</th>
                            <th class="px-3 md:px-6 py-2 md:py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider">Needed</th>
                            <th class="px-3 md:px-6 py-2 md:py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider">Packed</th>
                            <th class="px-3 md:px-6 py-2 md:py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Status</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($packingSummary as $summary)
                            <tr class="{{ $summary['complete'] ? 'bg-green-50' : '' }}">
                                <td class="px-3 md:px-6 py-3 md:py-4">
                                    <div class="text-sm md:text-base font-medium text-gray-900">{{ $summary['product_name'] }}</div>
                                    <div class="text-sm font-mono text-gray-600 mt-1">{{ $summary['barcode'] }}</div>
                                ...
                                <td class="px-3 md:px-6 py-3 md:py-4 text-center">
                                    <span class="text-base md:text-lg font-semibold text-gray-900">{{ $summary['boxes_needed'] }}</span>
                                    <span class="text-sm text-gray-500 block md:hidden">boxes</span>
                                </td>
                                <td class="px-3 md:px-6 py-3 md:py-4 text-center">
                                    <span class="text-base md:text-lg font-semibold text-blue-600">{{ $summary['boxes_packed'] }}</span>
                                    <span class="text-sm text-gray-500 block md:hidden">boxes</span>
                                </td>
    ...
    @if(!empty($packedBoxes))
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
            <div class="flex items-center justify-between mb-4 md:mb-6">
                <h3 class="text-lg md:text-xl font-semibold text-gray-900">Packed Boxes ({{ count($packedBoxes) }})</h3>
            </div>
                    ...
                    @foreach($packedBoxes as $box)
                        <div class="bg-green-50 border border-green-200 rounded-lg p-3">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex-1">
                                    <div class="font-mono text-base font-bold text-gray-900">{{ $box['box_code'] }}</div>
                                    <div class="text-base text-gray-700 mt-1">{{ $box['product_name'] }}</div>
                                </div>
                                @if($box['scanned_out'])
                                    <span class="px-2 py-1 text-sm font-medium rounded-full bg-green-100 text-green-800">
                                        ✓ Ready
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($packedBoxes as $box)
                                <tr class="bg-green-50">
                                    <td class="px-6 py-4 whitespace-nowrap font-mono text-base font-medium text-gray-900">
                                        {{ $box['box_code'] }}
                                    </td>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h3 class="text-lg md:text-xl font-semibold text-gray-900 mb-4">Select Transporter</h3>
        ...
        <div class="mb-4 md:mb-6">
            ...
            <select wire:model="transporter_id"
                    class="block w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-base md:text-lg">
        ...
        <button type="button"
                ...
                class="w-full px-4 md:px-6 py-3 md:py-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold text-base md:text-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            <svg class="w-5 h-5 md:w-6 md:h-6 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
            </svg>
            Ship Transfer
        </button>
        <p class="text-sm text-gray-500 text-center mt-2">Partial shipments allowed. Pack at least one box to ship.</p>
    </div>
</div>

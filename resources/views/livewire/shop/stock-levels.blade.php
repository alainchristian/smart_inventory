<div>
    <!-- Page Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Shop Stock Levels</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Full Boxes</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Partial Boxes</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($stockData as $data)
                        <tr class="{{ $data['is_low_stock'] ? 'bg-red-50' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 font-mono">{{ $data['product']->sku }}</div>
                                <div class="text-sm text-gray-500 font-mono">{{ $data['product']->barcode }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="text-sm font-semibold text-gray-900">{{ $data['full_boxes'] }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="text-sm font-semibold text-gray-900">{{ $data['partial_boxes'] }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="text-sm font-semibold text-blue-600">{{ number_format($data['total_items']) }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
    ...
    <div class="mt-6">
        <a href="{{ route('shop.transfers.request') }}"
           class="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold transition-colors shadow-sm">

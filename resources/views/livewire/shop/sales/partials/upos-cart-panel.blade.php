...
<div class="upos-cart-panel">
    ...
    <div class="upos-cart-header">
        ...
        <span class="upos-cart-title">{{ __('Cart') }}</span>
        @if(count($cart) > 0)
            <span class="upos-cart-badge">{{ count($cart) }}</span>
        @endif
        @if(count($cart) > 0)
            <button class="upos-cart-clear" wire:click="clearCart" wire:confirm="{{ __('Clear all items from cart?') }}">{{ __('Clear') }}</button>
        @endif
    </div>
    ...
    <div class="upos-cart-items">
            ...
            <div class="upos-cart-item-top">
                <div style="flex:1;min-width:0">
                    ...
                    @if(!empty($item['price_modified']))
                        <span style="font-size:10px;color:var(--amber);font-weight:600">{{ __('Price modified') }}</span>
                    @endif
                </div>
                <div class="upos-cart-item-actions">
                    <button class="upos-cart-item-btn" wire:click="openEditItem({{ $index }})" title="{{ __('Edit') }}">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button class="upos-cart-item-btn del" wire:click="removeCartItem({{ $index }})" title="{{ __('Remove') }}">
        ...
        <div class="upos-cart-empty">
            ...
            {{ __('No items yet.') }}<br>
            <span style="font-size:11px">{{ __('Tap a product to add it.') }}</span>
        </div>
        @endforelse
    </div>
    ...
    <div class="upos-cart-footer">
        @if(count($cart) > 0)
            <div class="upos-cart-subtotal">
                <span>{{ trans_choice(':count item|:count items', count($cart), ['count' => count($cart)]) }}</span>
            ...
            <div style="display:flex;align-items:center;gap:6px;padding:6px 0;margin-bottom:4px">
                ...
                <span style="font-size:11px;color:var(--amber);font-weight:600">{{ __('Contains warehouse items — fulfillment required') }}</span>
            </div>
            @endif
        @endif
        <div class="upos-cart-total">
            <span class="upos-cart-total-label">{{ __('Total') }}</span>

...
<div class="pos-root"
  ...
  style="height:100vh;display:flex;flex-direction:column;background:var(--bg);overflow:hidden;position:relative">
  ...
  <template x-for="t in toasts" :key="t.id">
    <div x-show="true"
         ...
         :style="`display:flex;align-items:center;gap:9px;padding:11px 16px;border-radius:12px;
                  ...
                  background:${t.type==='error'?'var(--red)':t.type==='warning'?'var(--amber)':'var(--green)'};color:#fff`">
...
<div style="position:fixed;inset:0;z-index:800;display:flex;align-items:center;justify-content:center;
            background:rgba(26,31,54,.45);backdrop-filter:blur(6px)">
  <div style="background:var(--surface);border-radius:20px;padding:32px 28px;width:440px;
    ...
    <div style="text-align:center;margin-bottom:24px">
      <div style="width:60px;height:60px;border-radius:16px;background:var(--accent-dim);
                  display:grid;place-items:center;margin:0 auto 16px">
        <svg width="28" height="28" fill="none" stroke="var(--accent)" stroke-width="2" viewBox="0 0 24 24">
          <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
        </svg>
      </div>
      <div style="font-size:20px;font-weight:800;color:var(--text);letter-spacing:-.4px">Select Shop</div>
      <div style="font-size:13px;color:var(--text-sub);margin-top:6px">Choose which shop to operate</div>
    </div>
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">
      @foreach($availableShops as $shop)
      <button wire:click="$set('shopId', {{ $shop['id'] }})"
              style="padding:14px 16px;border-radius:var(--r);text-align:left;cursor:pointer;transition:.15s;
                     border:2px solid {{ $shopId == $shop['id'] ? 'var(--accent)' : 'var(--border)' }};
                     background:{{ $shopId == $shop['id'] ? 'var(--accent-dim)' : 'var(--bg)' }}">
        <div style="font-size:14px;font-weight:700;color:{{ $shopId == $shop['id'] ? 'var(--accent)' : 'var(--text)' }}">
          {{ $shop['name'] }}
        </div>
        @if(isset($shop['address']))
        <div style="font-size:12px;color:var(--text-dim);margin-top:3px">{{ $shop['address'] }}</div>
        @endif
      </button>
      @endforeach
    </div>
    <button wire:click="$set('showShopSelectionModal', false)"
            style="width:100%;padding:14px;background:var(--accent);
                   color:#fff;border:none;border-radius:var(--r);font-size:15px;font-weight:700;cursor:pointer;
...
<div style="flex:1;display:grid;grid-template-columns:1fr 380px;overflow:hidden;min-height:0"
  ...
  <div style="display:flex;flex-direction:column;overflow:hidden;background:var(--bg)">
    ...
    <div style="padding:12px 14px;background:var(--surface);border-bottom:1px solid var(--border);flex-shrink:0;
      ...
      <div style="display:flex;gap:8px;align-items:stretch">
        ...
        <div style="flex:1;position:relative">
          <svg width="15" height="15" fill="none" stroke="var(--text-dim)" stroke-width="2" viewBox="0 0 24 24"
          ...
          <input wire:model.live="searchQuery"
                 ...
                 autocomplete="off"
                 style="width:100%;padding:10px 34px 10px 34px;border:1.5px solid var(--border);
          ...
          <div style="position:absolute;top:calc(100% + 6px);left:0;right:0;z-index:400;
            ...
            <div style="padding:8px 12px;font-size:10px;font-weight:700;letter-spacing:.6px;
                        ...
                        border-bottom:1px solid var(--border);background:var(--bg);border-radius:14px 14px 0 0">
            ...
            @endforeach
            @else
            <div style="padding:22px;text-align:center;color:var(--text-dim);font-size:13px">
        ...
        <div style="position:relative">
          <svg width="13" height="13" fill="none" stroke="var(--text-dim)" stroke-width="2" viewBox="0 0 24 24"
          ...
          <input wire:model.live="barcodeInput"
                 ...
                 placeholder="Scan..."
                 autocomplete="off"
                 style="width:120px;padding:10px 10px 10px 28px;border:1.5px solid var(--border);
                        border-radius:10px;font-size:12px;background:var(--surface);
                        color:var(--text);outline:none;font-family:var(--mono);transition:border-color .15s"
      ...
      <div wire:poll.2000ms
           style="margin-top:10px;padding:12px 14px;background:var(--green-dim);
        ...
        @if($qrAvailable)
        <div style="flex-shrink:0;background:var(--surface);padding:5px;border-radius:8px;
        ...
        @endif
        <div>
          <div style="font-size:12px;font-weight:800;color:var(--green);margin-bottom:3px">
            Phone Scanner Active
          </div>
          <div style="font-size:11px;color:var(--text-sub);margin-bottom:2px">
            Code: <span style="font-family:var(--mono);font-weight:700;color:var(--text)">{{ $scannerSession->session_code }}</span>
          </div>
    ...
    @php
        ...
        $stripColor   = $anyApproved ? 'var(--green)' : ($heldCount === 0 ? 'var(--accent)' : 'var(--amber)');
    @endphp
    ...
    <div style="flex:1;overflow-y:auto;padding:14px">
      ...
      <div class="pos-product-grid">
        ...
        @php
          ...
          $stockColor = $totalBoxes > 3 ? 'var(--green)' : ($totalBoxes >= 1 ? 'var(--amber)' : 'var(--red)');
          $stockBg    = $totalBoxes > 3 ? 'var(--green-dim)' : ($totalBoxes >= 1 ? 'var(--amber-dim)' : 'var(--red-dim)');
        @endphp
        <button wire:click="selectProduct({{ $p['id'] }})"
                class="pos-tile"
                style="background:var(--surface);border:1.5px solid var(--border);
                       border-radius:var(--r);padding:14px 13px;text-align:left;cursor:pointer;
                ...
                onmouseover="this.style.borderColor='var(--accent)';this.style.transform='translateY(-1px)';this.style.boxShadow='0 6px 20px var(--accent-glow)'"
                onmouseout="this.style.borderColor='var(--border)';this.style.transform='translateY(0)';this.style.boxShadow='var(--shadow-card)'">
          ...
          <span style="font-size:9px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;
                       color:var(--text-dim);background:var(--surface2);padding:2px 7px;
          ...
          @endif
          <div style="font-size:13px;font-weight:700;color:var(--text);line-height:1.35;
          ...
          <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-top:auto">
            <span style="font-size:15px;font-weight:800;color:var(--accent);font-family:var(--mono);line-height:1">
            ...
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:2px">
              ...
              <span style="font-size:9px;color:var(--text-dim)">{{ $stockItems }}&nbsp;items</span>
            </div>
          </div>
        </button>
        @endforeach
      ...
      <div style="height:100%;min-height:260px;display:flex;flex-direction:column;align-items:center;
        ...
        <div style="width:72px;height:72px;border-radius:20px;background:var(--surface);
                    display:grid;place-items:center;margin-bottom:20px;
                    border:2px dashed var(--border);box-shadow:0 2px 8px rgba(26,31,54,.05)">
          <svg width="34" height="34" fill="none" stroke="var(--border)" stroke-width="1.5" viewBox="0 0 24 24">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
        </div>
        <div style="font-size:16px;font-weight:700;color:var(--text-sub);margin-bottom:9px">
          Ready to sell
        </div>
  ...
  <div class="pos-cart-panel"
       style="display:flex;flex-direction:column;background:var(--surface);
      ...
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:9px">
          <div style="width:32px;height:32px;border-radius:9px;background:var(--surface2);
                      display:grid;place-items:center">
            <svg width="15" height="15" fill="none" stroke="var(--text-sub)" stroke-width="2.5" viewBox="0 0 24 24">
              <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
              <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
            </svg>
          </div>
        ...
        <button wire:click="clearCart"
                ...
                style="font-size:11px;font-weight:700;color:var(--red);background:var(--red-dim);
                       border:1px solid var(--red-dim);padding:4px 10px;border-radius:7px;cursor:pointer">
          Clear all
        </button>
        @endif
      </div>
    ...
    <div style="margin:0;padding:10px 14px;background:var(--amber-dim);border-bottom:1.5px solid var(--amber);
                flex-shrink:0;display:flex;align-items:center;justify-content:space-between;gap:8px">
      <div style="display:flex;align-items:center;gap:7px">
        <svg width="14" height="14" fill="none" stroke="var(--amber)" stroke-width="2" viewBox="0 0 24 24">
          <path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
          <line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
        </svg>
        <span style="font-size:11px;font-weight:700;color:var(--amber)">
      ...
      <button wire:click="holdSale"
              style="padding:5px 11px;border-radius:7px;border:none;cursor:pointer;
                     background:var(--green);color:#fff;font-size:11px;font-weight:700;white-space:nowrap">
        Hold for Approval
      </button>
    </div>
    @endif
    ...
    <div style="margin:0;padding:10px 14px;background:var(--green-dim);border-bottom:1.5px solid var(--green);
                flex-shrink:0;display:flex;align-items:center;justify-content:space-between;gap:8px">
      <div style="display:flex;align-items:center;gap:7px">
      ...
      <button wire:click="holdSale"
              style="padding:5px 11px;border-radius:7px;border:1px solid var(--green);cursor:pointer;
                     background:var(--surface);color:var(--green);font-size:11px;font-weight:700;white-space:nowrap">
        Park Back
      </button>
    </div>
    @endif
    ...
    <div style="flex:1;overflow-y:auto;padding:10px 12px 6px">
        ...
        <div style="display:flex;align-items:flex-start;gap:7px;margin-bottom:7px">
          ...
          <div style="display:flex;gap:4px;flex-shrink:0">
            <button wire:click="openEditItem({{ $index }})"
                    style="width:28px;height:28px;border-radius:7px;background:var(--accent-dim);
            ...
            <button wire:click="removeCartItem({{ $index }})"
                    style="width:28px;height:28px;border-radius:7px;background:var(--red-dim);
      ...
      <div style="min-height:180px;display:flex;flex-direction:column;
        ...
        <svg width="40" height="40" fill="none" stroke="var(--border)" stroke-width="1.5" viewBox="0 0 24 24"
             style="margin-bottom:14px">
          <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
          <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
        </svg>
        <div style="font-size:13px;font-weight:700;color:var(--text-dim)">Cart is empty</div>
        <div style="font-size:12px;color:var(--text-dim);margin-top:5px">Tap a product to add it</div>
      </div>
      @endforelse
    </div>

    {{-- Cart footer --}}
    <div style="border-top:1.5px solid var(--border);padding:14px 16px 16px;flex-shrink:0;background:var(--surface)">
      @if(!empty($cart))
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;
          ...
          <span style="font-size:26px;font-weight:800;color:var(--accent);font-family:var(--mono);line-height:1">
            {{ number_format($cartTotal) }}<span style="font-size:12px;font-weight:600;color:var(--text-sub)"> RWF</span>
          </span>
        </div>
      </div>
      @endif
...
<div class="pos-cart-drawer-overlay"
  ...
  <div class="pos-cart-drawer"
       ...
       style="position:absolute;bottom:0;left:0;right:0;background:var(--surface);
              ...
              box-shadow:0 -8px 40px rgba(26,31,54,.2)">

    {{-- Drawer handle --}}
    <div style="padding:14px 0 8px;display:flex;justify-content:center;flex-shrink:0">
      <div style="width:44px;height:5px;border-radius:3px;background:var(--border)"></div>
    </div>
    ...
    <div style="padding:0 18px 14px;display:flex;align-items:center;justify-content:space-between;
      ...
      <div style="display:flex;align-items:center;gap:8px">
        <span style="font-size:17px;font-weight:800;color:var(--text)">Cart</span>
        @if(!empty($cart))
        <span style="background:var(--accent);color:#fff;font-size:10px;font-weight:800;
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <button @click="cartOpen = false"
                style="width:32px;height:32px;border-radius:9px;background:var(--surface2);
                       border:none;cursor:pointer;display:grid;place-items:center;color:var(--text-sub)">
          <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
            <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
          </svg>
        </button>
    ...
    <div style="flex:1;overflow-y:auto;padding:12px 18px 6px">
      ...
      <div wire:key="mob-ci-{{ $index }}"
        ...
        <div style="display:flex;align-items:center;justify-content:space-between">
          ...
          <span style="font-size:15px;font-weight:800;color:var(--text);font-family:var(--mono)">
            {{ number_format($item['line_total']) }}
            <span style="font-size:9px;color:var(--text-dim)">RWF</span>
          </span>
        </div>
      </div>
      @empty
      <div style="padding:32px 0;text-align:center;color:var(--text-dim)">
        <svg width="40" height="40" fill="none" stroke="var(--border)" stroke-width="1.5" viewBox="0 0 24 24"
    ...
    {{-- Drawer footer --}}
    <div style="border-top:1.5px solid var(--border);padding:14px 18px;background:var(--surface);flex-shrink:0">
      @if(!empty($cart))
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
        ...
        <span style="font-size:28px;font-weight:800;color:var(--accent);font-family:var(--mono);line-height:1">
          {{ number_format($cartTotal) }}<span style="font-size:12px;color:var(--text-sub)"> RWF</span>
        </span>
      </div>
      @endif
      @if(!empty($cart))
      ...
      <button wire:click="openCheckout" @click="cartOpen = false"
              ...
              style="width:100%;padding:16px;
                     ...
                     cursor:{{ empty($cart) ? 'not-allowed' : 'pointer' }};
                     box-shadow:{{ empty($cart) ? 'none' : '0 4px 20px var(--accent-glow)' }};
                     font-family:var(--font)">
...
.sm-overlay {
    ...
    justify-content:center;background:rgba(26,31,54,.45);backdrop-filter:blur(6px);
    padding:16px;
}
.sm-card {
...
.sm-modified-badge {
    ...
    background:var(--amber-dim);padding:2px 7px;border-radius:5px;
...
.sm-total.modified { border-color:var(--amber);background:var(--amber-dim) }
...
.sm-confirm-btn {
    ...
    display:flex;align-items:center;justify-content:center;gap:7px;
    box-shadow:0 4px 14px var(--accent-glow);transition:opacity var(--tr);
}
...
.co-customer-clear:hover { background:var(--red-dim);filter:brightness(.9) }
...
/* Credit warning */
.co-credit-warning {
    padding:10px 13px;background:var(--amber-dim);
...
.co-complete-btn {
    ...
    box-shadow:0 5px 18px var(--green-glow);transition:opacity var(--tr);
}
      ...
      <div class="co-col"
        ...
        <div class="co-pay-list">
          ...
          <div class="co-pay-row" :class="card > 0 ? 'is-active' : ''">
            <div class="co-pay-icon" style="background:var(--accent-dim)">
              <svg width="14" height="14" fill="none" stroke="var(--accent)" stroke-width="2" viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
            </div>
          ...
          <div class="co-pay-row" :class="momo > 0 ? 'is-active' : ''">
            <div class="co-pay-icon" style="background:var(--green-dim)">
              <svg width="14" height="14" fill="none" stroke="var(--green)" stroke-width="2" viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18" stroke-linecap="round" stroke-width="2.5"/></svg>
            </div>
            <div class="co-pay-meta">
          ...
          <div class="co-pay-row" :class="bank > 0 ? 'is-active' : ''">
            <div class="co-pay-icon" style="background:var(--accent-dim)">
              <svg width="14" height="14" fill="none" stroke="var(--accent)" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 10h16v11H4zM2 7l10-4 10 4M8 14v3m4-3v3m4-3v3"/></svg>
            </div>
            <div class="co-pay-meta">
          ...
          <div class="co-pay-row" :class="credit > 0 ? 'is-active' : ''">
            <div class="co-pay-icon" style="background:var(--amber-dim)">
              <svg width="14" height="14" fill="none" stroke="var(--amber)" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
            </div>
            <div class="co-pay-meta">
        ...
        <div class="co-cash-row" style="border:1px solid var(--border);border-radius:13px;margin-bottom:14px;">
          <div class="co-pay-icon" style="background:var(--green-dim)">
            <svg width="14" height="14" fill="none" stroke="var(--green)" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="3"/><path stroke-linecap="round" d="M6 10h.01M18 14h.01"/></svg>
          </div>
          <div class="co-pay-meta">
          ...
          <div class="co-cash-display" :style="cash < 0 ? 'border-color:var(--red);color:var(--red);background:var(--red-dim)' : ''">

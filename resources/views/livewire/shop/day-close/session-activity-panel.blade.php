    ...
    <div style="padding:10px 10px 0;" x-data="{ open: false }" @click.outside="open = false">
        ...
        <button @click="open = !open"
                style="width:100%;display:flex;align-items:center;justify-content:space-between;
                       ...
                       background:var(--surface-raised);cursor:pointer;transition:border-color 0.15s;"
            ...
            <div style="display:flex;align-items:center;gap:8px;min-width:0;">
                @if ($selectedSession)
                    ...
                    <span style="font-size:12px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                        ...
                        <span style="font-weight:400;color:var(--text-faint);font-size:11px;margin-left:4px;">
                    ...
                    @endif
                @else
                    <span style="font-size:12px;color:var(--text-faint);">Select a session…</span>
                @endif
            </div>
            <svg class="transition-transform duration-200" :class="{ 'rotate-180': open }"
                 style="width:14px;height:14px;color:var(--text-faint);flex-shrink:0;"
        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100"
            ...
            {{-- Search --}}
            <div style="padding:8px 8px 6px;border-bottom:1px solid var(--border);background:var(--surface-raised);">
            ...
            <div style="max-height:280px;overflow-y:auto;-webkit-overflow-scrolling:touch;">
                @if ($sessions->isEmpty())
                    <div style="padding:20px;text-align:center;font-size:12px;color:var(--text-faint);">No sessions found.</div>
                @else
                    ...
                    @foreach ($sessions as $sess)
                        ...
                        <button wire:click="selectSession({{ $sess->id }})"
                                ...
                                onmouseover="if(!{{ $isSelected ? 'true' : 'false' }}) this.style.background='var(--surface-raised)'"
                            ...
                            <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">
                                ...
                                @elseif ($sess->isLocked())
                                    <span style="font-size:9px;padding:1px 6px;border-radius:999px;
                                                 background:var(--surface-raised);color:#94a3b8;border:1px solid var(--border);">Locked</span>
                                @else
                    ...
                    @if ($sessions->count() === 60)
                        <div style="padding:8px 12px;text-align:center;font-size:11px;color:var(--text-faint);

<div>
    ...
    <div style="display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap;">
        <div>
            ...
            <div style="font-size:10px;margin-top:4px;color:{{ $sourceAvail > 0 ? $sourceColor : 'var(--red)' }};">
                {{ __(':amount RWF available', ['amount' => number_format($sourceAvail)]) }}
            </div>
        </div>
        <input type="text"
               ...
               enterkeyhint="done"
               placeholder="{{ __('Slip / ref number') }}"
    ...
    <div style="margin-top:14px;">
        <button wire:click="saveDeposit"
            ...
            <span wire:loading.remove wire:target="saveDeposit">{{ __('Record Deposit') }}</span>
            <span wire:loading wire:target="saveDeposit" style="display:none;">{{ __('Saving…') }}</span>
        </button>
    </div>
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;
                        ...
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                            <span style="padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700;
                                ...
                                {{ ($deposit->source ?? 'cash') === 'mobile_money' ? __('MoMo') : __('Cash') }}
                            </span>
                        ...
                        <button wire:click="voidDeposit({{ $deposit->id }})"
                                wire:confirm="{{ __('Void this deposit of :amount RWF?', ['amount' => number_format($deposit->amount)]) }}"
